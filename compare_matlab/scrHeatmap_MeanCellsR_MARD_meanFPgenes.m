...
meanCellsR=squeeze(mean(corrCells(:,2,vSelect)));
...
h=heatmap({'b'}, label1, round(meanCellsR,3), 'FontSize',32, 'ColorbarVisible','off');
...
h.XDisplayLabels={''};
annotation(gcf, 'textbox', [xS yS+yW xW 0.1], 'String', 'mean per-cell R', 'LineStyle', 'none', ...
            'FontSize',32, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'Center', 'FontWeight','bold');
...
h=heatmap({'a'}, label1, round(FPgenes,1), 'FontSize',32, 'ColorbarVisible','off');
h.Colormap = h.Colormap(end:-1:1,:);
...
h.XDisplayLabels={''};
annotation(gcf, 'textbox', [xS+xW+xB+xW+xB yS+yW xW 0.12], 'String', [compareFPgenes ' genes per cell'], 'LineStyle', 'none', ...
            'FontSize',32, 'HorizontalAlignment', 'Center', 'FontWeight','bold','VerticalAlignment', 'bottom');
%%
saveas(gcf, [savePrefix 'Heatmap-MeanCellsR-MARD-meanFPgenes' '.png']);